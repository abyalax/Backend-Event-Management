...
export class TicketLockService {
  private readonly logger = new Logger(TicketLockService.name);
  ...
  async lockTicketQuota(ticketId: string, orderId: string, quantity: number): Promise<boolean> {
    ...
    try {
      ...
      this.logger.log(`Successfully locked ${quantity} tickets for ticket ${ticketId}, order ${orderId}. Remaining quota: ${remainingQuota}`);
      return true;
    } catch (error) {
  ...
  async releaseTicketQuota(ticketId: string, orderId: string): Promise<void> {
    ...
    try {
      ...
      if (lockedQuantity) {
        ...
        this.logger.log(`Released ${lockedQuantity} tickets for ticket ${ticketId}, order ${orderId}`);
      }
    } catch (error) {
  ...
  async setInitialQuota(ticketId: string, totalQuota: number): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Set initial quota ${totalQuota} for ticket ${ticketId}`);
    } catch (error) {
